...
Instead of waiting for the full response to be received before showing it to the user, the chat app now streams the response generation in real time. This makes for shorter wait times and removes the need to use `{waiter}`. Fix #85 

## Bug fixes

- The welcome message is no longer consumed by the chat history. Fix #80 
- Errors in requests now point to the OpenAI documentation. Fix #72 
