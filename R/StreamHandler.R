...
#'
#' Because `curl::curl_fetch_stream` blocks the R console until the stream
...
StreamHandler <- R6::R6Class(
  ...
  public = list(
    ...
    #' @field current_value The content of the stream. It updates constantly
    #'   until the stream ends.
    current_value = NULL,

    #' @field chunks  The list of chunks streamed. It updates constantly until
    #'   the stream ends.
    chunks = list(),
    ...
    #' @field user_message  The `user_prompt` provided at initialization after
    #'   being formatted with markdown.
    user_message = NULL,

    #' @description Start a StreamHandler. Recommended to be assigned to the
    #'   `stream_handler` name.
    ...
    #' @description The main reason this class exists. It reduces to stream to
    #'   chunks and its current value. If the object finds a shiny session will
    #'   send a `render-stream` message to JS.
    ...
    #' @description Extract the message content as a message ready to be styled
    #'   or appended to the chat history. Useful after the stream ends.
