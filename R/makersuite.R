#' Base for a request to the MakerSuite API
#'
#' This function sends a request to a specific MakerSuite API endpoint and
#' authenticates with an API key.
#'
#' @param model character string specifying a MakerSuite API model
#' @param key String containing a MakerSuite API key. Defaults to the
#'   PALM_API_KEY environmental variable if not specified.
#' @keywords makersuite, api, authentication
#' @return An httr2 request object
request_base_ms <- function(model, key = Sys.getenv("PALM_API_KEY")) {
...
#' A function that sends a request to the MakerSuite API and returns the
#' response.
#'
...
#' @param key String containing a MakerSuite API key. Defaults
#'   to the PALM_API_KEY environmental variable if not specified.
#'
#' @return The response from the API.
#'
query_api_ms <- function(model,
                         request_body,
                         key = Sys.getenv("PALM_API_KEY")) {
  response <- request_base_ms(model, key) %>%
  ...
  if (httr2::resp_is_error(response)) {
    ...
    cli::cli_abort(message = c(
      "x" = "MakerSuite API request failed. Error {status} - {description}",
      "i" = "Visit the MakerSuite API documentation for more details"
    ))
  }
...
#' Generate text completions using MakerSuite's API
#'
...
#' @param key The API key for accessing MakerSuite's API. By default, the
...
#' }
#' @export
create_completion_ms <- function(prompt,
                                 model = "text-bison-001",
                                 key = Sys.getenv("PALM_API_KEY"),
                                 temperature = 0.5,
                                 candidate_count = 1) {
  # The request body for the MakerSuite API should be a list with the 'prompt',
  # 'temperature', and 'candidate_count' fields set
  prompt <- list(text = prompt)
  request_body <- list(prompt = prompt,
                       temperature = temperature,
                       candidate_count = candidate_count)
  response <- query_api_ms(model = model,
