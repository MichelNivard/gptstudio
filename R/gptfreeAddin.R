  ...
  ui <- miniUI::miniPage(
    ...
    miniUI::miniContentPanel(
     shiny::fillCol(flex = c(1,1),
      shiny::fillRow(flex = c(1,1,3),
      shiny::radioButtons(
                          inputId = "model",
                          label = "What OpenAI model do you want to use to edit?",
                          choices=list("Use text model" = "text-davinci-edit-001", "Use code model (alpha)" = "code-davinci-edit-001"),
                          width="90%"),
      shiny::sliderInput(inputId = "temperature",
